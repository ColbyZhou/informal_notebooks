...
def random_mini_batches(XE, R1E, R2E, mini_batch_size = 10, seed = 42, epoc_idx = 0, dump_data = None):
    ...
    dump_data["perm_" + str(epoc_idx)] = permutation
...
dump_data = {"k": k, "u1": u1, "X": X, "eps1": eps1, "eps2": eps2, "split": split}
print(k)
print(u1)
print(X)
print(eps1)
...
with tf.Session() as sess:
    ...
    for it in range(epoch):
        ...
        minibatches = random_mini_batches(X_train, Y1_train, Y2_train, mb_size, seed=seed, epoc_idx=it, dump_data=dump_data)

        for minibatch in minibatches:
            XE, YE1, YE2  = minibatch

            # Update GradNorm and model weights
            l, l_t1, l_t2, we1, we2, g1, g2, c1, c2, l_hat_1_val, l_hat_2_val, l_hat_avg_val, _ = \
                    sess.run([loss_op, l1, l2, w1, w2, G1, G2, C1, C2, l_hat_1, l_hat_2, l_hat_avg, [train_step, loss_step, update_op]],
                              feed_dict={X_: XE,
                                         Y1_: np.reshape(YE1, [-1, 1]),
                                         Y2_: np.reshape(YE2, [-1, 1]),
                                         keep_prob: 0.5})

            print("\t".join(map(str, [we1, we2, g1, g2, c1, c2, l_hat_1_val, l_hat_2_val, l_hat_avg_val])))
    ...
    plt.savefig('train_weight.jpg')
    plt.show()

    plt.plot(np.squeeze(g1s), '-r')
    plt.ylabel('Grad')
    plt.xlabel('steps')
    plt.title('Grads')
    plt.gca().legend(('g1'))
    plt.savefig('train_grad_g1.jpg')
    plt.show()

    plt.plot(np.squeeze(g2s), '-b')
    plt.ylabel('Grad')
    plt.xlabel('steps')
    plt.title('Grads')
    plt.gca().legend(('g2'))
    plt.savefig('train_grad_g2.jpg')
    plt.show()
    ...
    #print("grads")
    #for (a, b, c) in zip(g1s, g2s, c1s):
    #    d = a + b
    #    print("%f, %f, %f, %f" % (a, b, c, d))


with open("demo_data.pickle", 'wb') as file:
